$(document).ready ->
  ...
  # Begin of code for handling year in music release resource    
  ...
  # Begin of code for handling year in music track resource    
  ...
  $(document.body).on "click", ".year_in_review_music_track_link", (event) ->
    $this = $(this)
    
    $.ajax(url: $this.attr('href'), type: "GET", dataType: "html").success (data) ->
      $($this.data("replace")).html(data)
      window.make_year_in_review_music_tracks_sortable()
      
    event.preventDefault()  
        
  first_position = $('#year_in_review_music_tracks li:first').data('position')
  last_position = $('#year_in_review_music_tracks li:last').data('position')
